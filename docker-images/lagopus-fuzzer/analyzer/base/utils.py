...
def filter_file_list(file_list):
  ...
  if len(filtered_file_list) != len(file_list):
    print('Filtered file list (%s) from (%s).' % (str(filtered_file_list),
                                                     str(file_list)))
...
def get_process_ids(process_id, recursive=True):
  ...
  try:
    ...
    print('Failed to get process children.')
    return []
...
def is_binary_file(file_path, bytes_to_read=1024):
  ...
  try:
    ...
    print('Could not read file %s in is_binary_file.' % file_path)
    return None
...
def read_data_from_file(file_path, eval_data=True, default=None):
  ...
  for _ in range(retry_limit):
    try:
      ...
      file_content = None
      print('Error occurred while reading %s, retrying.' % file_path)
      time.sleep(random.uniform(1, failure_wait_interval))
      continue

  if file_content is None:
    print('Failed to read data from file %s.' % file_path)
    return None
...
def write_data_to_file(content, file_path, append=False):
  ...
  for _ in range(retry_limit + 1):
    try:
      ...
      print('Error occurred while writing %s, retrying.' % file_path)
      time.sleep(random.uniform(1, failure_wait_interval))
      continue
  ...
  print('Failed to write data to file %s.' % file_path)
