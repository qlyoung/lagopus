...
def ignore_stacktrace(crash_stacktrace):
  ...
  search_excludes = environment.get('SEARCH_EXCLUDES')
...
def is_crash(return_code, console_output):
  ...
  crash_signature = environment.get('CRASH_SIGNATURE')
  if crash_signature:
...
def is_security_issue(crash_stacktrace, crash_type, crash_address):
  ...
  if crash_type == 'CHECK failure':
    ...
    # DCHECKs from CHECKs.
    checks_have_security_implication = environment.get(
  ...
  if crash_type == 'ASSERT' or 'ASSERTION FAILED' in crash_stacktrace:
    asserts_have_security_implication = environment.get(
...
def has_ubsan_error(stacktrace):
  ...
  ubsan_ignores_file_path = environment.get('UBSAN_SUPPRESSIONS_FILE')
