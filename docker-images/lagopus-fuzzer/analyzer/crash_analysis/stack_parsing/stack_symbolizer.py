...
def strip_from_right(s, suffix):
    return re.sub('{}$'.format(suffix), '', s)
...
def chrome_dsym_hints(binary):
  ...
  innermost_bundle_dir = strip_from_right(innermost_bundle_dir, '.app')
  innermost_bundle_dir = strip_from_right(innermost_bundle_dir,
                                                '.framework')
...
class LLVMSymbolizer(Symbolizer):
  ...
  def symbolize(self, addr, binary, offset):
    ...
    try:
      ...
      print('Symbolization using llvm-symbolizer failed for: "%s".' %
                     symbolizer_input)
      result = []
    if not result:
      result = None
...
class Addr2LineSymbolizer(Symbolizer):
  ...
  def symbolize(self, addr, binary, offset):
    ...
    try:
      ...
      print('Symbolization using addr2line failed for: "%s %s".' %
                     (binary, str(offset)))
      function_name = ''
      file_name = ''
...
class DarwinSymbolizer(Symbolizer):
  ...
  def symbolize(self, addr, binary, offset):
    ...
    try:
      ...
      print('Symbolization using atos failed for: "%s %s".' %
                     (binary, str(offset)))
...
def symbolize_stacktrace(unsymbolized_crash_stacktrace,
  ...
  llvm_symbolizer_path = environment.get('LLVM_SYMBOLIZER_PATH')
  if not llvm_symbolizer_path:
    return unsymbolized_crash_stacktrace

  print("zoomin")
  print('st: {}'.format(unsymbolized_crash_stacktrace))
